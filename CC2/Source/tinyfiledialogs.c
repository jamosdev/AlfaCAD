...
int tinyfd_verbose = 0; //1 ; /* on unix: prints the command line calls */ //0
...
wchar_t* tinyfd_mbcsTo16(char const* aMbcsString)
{
    ...
    {
        lMbcsString = (wchar_t*)malloc(1 * sizeof(wchar_t));
        ...
        wchar_t const * aSingleFilterDescription) /* NULL or "image files" */
{
        ...
        ofn.Flags = OFN_OVERWRITEPROMPT | OFN_NOCHANGEDIR | OFN_PATHMUSTEXIST | OFN_OVERWRITEPROMPT;
        ofn.nFileOffset = 0;
        ...
        int aAllowMultipleSelects) /* 0 or 1 ; -1 to free allocated memory and return */
{
        ...
        ofn.lpstrInitialDir = wcslen(lDirname) ? lDirname : NULL;
        ofn.lpstrTitle = aTitle && wcslen(aTitle) ? aTitle : NULL;
        ofn.Flags = OFN_EXPLORER | OFN_NOCHANGEDIR | OFN_PATHMUSTEXIST | OFN_OVERWRITEPROMPT; // | OFN_FILEMUSTEXIST;
        ofn.nFileOffset = 0;
        ofn.nFileExtension = 0;
...
static char* FileNameDialogWinGui(
    char const* aTitle, /*  NULL or "" */
    char const* aDefaultPathAndFile, /*  NULL or "" */
    int aNumOfFilterPatterns, /* 0 */
    char const* const* aFilterPatterns, /* NULL or {"*.jpg","*.png"} */
    ...
    ) /* 0 or 1 */
{
    ...
    {
        if (aTitle && !strcmp(aTitle, "tinyfd_query")) { strcpy(tinyfd_response, "windows"); return (char*)1; }
        p = FileNameDialogWinGui(aTitle, aDefaultPathAndFile, aNumOfFilterPatterns,
            (char const* const*)aFilterPatterns, aSingleFilterDescription, aAllowMultipleSelects);
    }
    else if (dialogPresent())
    {
    ...
    /*  here we do not check if file exists, it can be also newrly cretaed file
    if (aAllowMultipleSelects && strchr(p, '|'))
    {
        p = ensureFilesExist((char*)p, p);
    }
...
int tfd_yadPresent(void)
{
   ...
   if (lYadPresent < 0)
   {
      lYadPresent = detectPresence("./yad4alfa");
   }
...
int tfd_kdialogPresent__(void)
{
    ...
    if ( lKdialogPresent < 0 )
    {
        ...
        {
            lIn = popen( "./kdialog4alfa --attach 2>&1" , "r" ) ;
            if ( fgets( lBuff , sizeof( lBuff ) , lIn ) != NULL )
            {
            ...
            {
                lKdialogPresent = 1 ;
                lIn = popen( "./kdialog4alfa --passivepopup 2>&1" , "r" ) ;
                if ( fgets( lBuff , sizeof( lBuff ) , lIn ) != NULL )
                {
                    if ( ! strstr( "Unknown" , lBuff ) )
                    {
...
int tfd_kdialogPresent(void)   //REPLACED
{
	static int lKdialogPresent = -1 ;
	char lBuff[MAX_PATH_OR_CMD] ;
	FILE * lIn ;
...
int tfd_kdialogDirPresent(void)
{
    static int lKdialogDirPresent = -1 ;
    char lBuff[MAX_PATH_OR_CMD] ;
    FILE * lIn ;
    char * lDesktop;

    if ( lKdialogDirPresent < 0 )
    {
        /*
		if ( tfd_zenityPresent() )
		{
        ...
        */
        lKdialogDirPresent = detectPresence("./kdialogDir4alfa") ;
        //lKdialogDirPresent = 1 ;
        if ( lKdialogDirPresent && !getenv("SSH_TTY") )
        {
            lIn = popen( "./kdialogDir4alfa --attach 2>&1" , "r" ) ;
            if ( fgets( lBuff , sizeof( lBuff ) , lIn ) != NULL )
            {
                if ( ! strstr( "Unknown" , lBuff ) )
                {
                    lKdialogDirPresent = 2 ;
                    if (tinyfd_verbose) printf("kdialog-attach %d\n", lKdialogDirPresent);
                }
            }
            pclose( lIn ) ;

            if (lKdialogDirPresent == 2)
            {
                lKdialogDirPresent = 1 ;
                lIn = popen( "./kdialogDir4alfa --passivepopup 2>&1" , "r" ) ;
                if ( fgets( lBuff , sizeof( lBuff ) , lIn ) != NULL )
                {
                    if ( ! strstr( "Unknown" , lBuff ) )
                    {
    ...
    return graphicMode() ? lKdialogDirPresent : 0 ;
}
...
char * tinyfd_editBox(
        char const * aTitle , /* NULL or "" */
        char const * aMessage , /* NULL or "" (\n and \t have no effect) */
        int const * aDefaultInput, /* params*/
        char const *fontface,
        char const *fontfile,
        char const *fontsize,
...
{
    static char lBuff[MAX_TEXT_SIZE];   //MAX_TEXT_SIZE  MAX_PATH_OR_CMD
    static char lBuff_ex[MAX_PATH_OR_CMD];  //MAX_TEXT_SIZE
    char * lDialogString = NULL;
    char * lpDialogString;
    FILE * lIn , *lInt;
    int lResult ;
    ...
    edit_file = (bool)(*aDefaultInput & 0x100) / 0x100;
    new_bold = (bool)(*aDefaultInput & 2)/2;
    new_italics = (bool)(*aDefaultInput & 4)/4;
    new_underline = (bool)(*aDefaultInput & 8)/8;
    ...
    */
    ...
    else if ( tfd_kdialogPresent())
    {
        ...
        if (*single==0)  strcat(lDialogString, " --textinputbox ") ;  //text
        else strcat(lDialogString, " --inputbox ") ;  //text
        ...
        //strcat(lDialogString,  "640x400+400+250") ;
        strcat(lDialogString,  editbox_geometry) ;
        strcat(lDialogString, "\"") ;

        strcat(lDialogString, " --fontface \"") ;
        strcat(lDialogString,  fontface) ;
        strcat(lDialogString, "\"") ;

        strcat(lDialogString, " --fontfile \"") ;
        strcat(lDialogString,  fontfile) ;
        strcat(lDialogString, "\"") ;

        //sprintf(font_size, "%d", *fontsize);
        strcat(lDialogString, " --fontsize \"") ;
        strcat(lDialogString,  fontsize) ;
        strcat(lDialogString, "\"") ;


        //sprintf(font_width, "%d", *fontwidth);
        strcat(lDialogString, " --fontwidth \"") ;
        strcat(lDialogString,  fontwidth) ;
        strcat(lDialogString, "\"") ;

        //sprintf(font_width, "%d", *fontwidth);
        strcat(lDialogString, " --etype \"") ;
        strcat(lDialogString,  etype) ;
        strcat(lDialogString, "\"") ;

        //sprintf(params, "%d", *aDefaultInput);
    ...
    else if (( tfd_zenityPresent() || tfd_matedialogPresent() || tfd_shellementaryPresent() || tfd_qarmaPresent() ))
    {
        ...
        strcat( lDialogString ," --text-info --editable"); //--entry" ) ;
    ...
    else if (( gxmessagePresent() || gmessagePresent() ) && (1==2))
    {
        if ( gxmessagePresent() ) {
...
frontmost of process \\\"Python\\\" to true' ''');");
    ...
    else if ( gdialogPresent() || xdialogPresent() || dialogName() || whiptailPresent() )
    {
        ...
        if ( lWasGraphicDialog )
        {
            strcat(lDialogString,") >/tmp/tinyfd.txt; \
        if [ $? = 0 ];then tinyfdBool=1;else tinyfdBool=0;fi;\
        tinyfdRes=$(cat /tmp/tinyfd.txt);echo $tinyfdBool$tinyfdRes") ;
        }
        else
        {
            strcat(lDialogString,">/dev/tty ) >/tmp/tinyfd.txt; \
        if [ $? = 0 ];then tinyfdBool=1;else tinyfdBool=0;fi;\
        tinyfdRes=$(cat /tmp/tinyfd.txt);echo $tinyfdBool$tinyfdRes") ;
    ...
    char const * aSingleFilterDescription ) /* NULL or "image files" */
{
    ...
    int aNumOfFilterPatterns=0;
    char *ptr0, *ptr;
    char current_mask[32];

    char aFilterPatterns[16][32];

    strcpy(current_mask, aFilterPatterns0);
    ptr0=current_mask;
...
char * tinyfd_FileNameDialog(   //TO CHECK
        char const * aTitle , /* NULL or "" */
        char const * aDefaultPathAndFile , /* NULL or "" */
        int aNumOfFilterPatterns0 , /* 0 */
        //char const * const * aFilterPatterns , /* NULL or {"*.jpg","*.png"} */
        ...
        ) /* 0 or 1 */
{
    ...
    char *XDG_CURRENT_DESKTOP=get_XDG_CURRENT_DESKTOP();
    ...
    else if ( tfd_kdialogPresent()) // && (strcmp(XDG_CURRENT_DESKTOP, "KDE")==0))
    {
        ...
        //kdialog --getopenfilename "/home/marek/ALFACAD64/Drawings" 'Alfa Files (*.alf)' --title "Load drawing from file"
        ...
        char const * aDefaultPath ) /* "" */
{
        ...
        {
           if (aTitle && !strcmp(aTitle, "tinyfd_query")) { strcpy(tinyfd_response, "./yad4alfa"); return (char*)1; }
           strcpy(lDialogString, "./yad4alfa --file-selection --directory");
           if (aTitle && strlen(aTitle))
           {
              strcat(lDialogString, " --title=\"");
              strcat(lDialogString, aTitle);
...
frontmost of process \\\"Python\\\" to true' ''');");
        ...
        else if ( (xdialogPresent() || dialogName()) && (1==1))
        {
                ...
                strcat( lDialogString , "--dselect --auto-placement \"" ) ;   //--under-mouse
                if ( aDefaultPath && strlen(aDefaultPath) )
                {
                        strcat(lDialogString, aDefaultPath) ;
...
"-e 'set mystring to mystring & \" \" & ((item i of mycolor) div 256 as integer) as string' " );
        ...
        {
           if (aTitle && !strcmp(aTitle, "tinyfd_query")) { strcpy(tinyfd_response, "./yad4alfa"); return (char*)1; }
           strcpy(lDialogString, "./yad4alfa --color");
